  ...
  for (let table of tablesToSync) {
    ...
    co(function *() {
      while (!tableDone) {
        ...
        let pc = ((recordsProcessed / totalRecords) * 100).toFixed(1)
        process.stdout.write(` RECORDS SYNCHRONIZED: ${recordsProcessed} | ${rps} sec. | %${pc} | created ${created} | updated ${updated} | deleted ${deleted}          \r`)
