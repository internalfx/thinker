  ...
  if (!indexesOnly) {
    for (let table of Object.keys(tablesManager)) {
      ...
      if (companyIds) {
        ...
        companyMessage = `by Companies: ${companyIds}`
      } else {
      ...
      logger.info(`Synchronizing from ${totalRecordsSource} records in ${table} from DB ${sourceDB} with ${totalRecordsTarget ? `${totalRecordsTarget} on ` : ''}${table} from DB ${targetDB} ${companyMessage ? companyMessage : ''}...      `)
      let sourceCursor
      let targetCursor
