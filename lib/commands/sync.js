...
module.exports = function *(argv) {
  ...
  let targetTableName = argv.st ? argv.st : argv.targetTable ? argv.targetTable : null
  ...
  if (`${sourceHost}:${sourcePort}` === `${targetHost}:${targetPort}` && sourceDB === targetDB) {
    if (pickTables && pickTables.length === 1 && pickTables[0] === "files" && targetTableName) {
      ...
      // Allow "--pt files" only to copy data from same server to new table named `targetTableName`
      COPY_TABLE_MODE = true;
      console.log(`Copying "files" table from ${sourceHost}:${sourcePort} to SAME server as a new table named: "${targetTableName}"`)
    } else {
  ...
  if (!indexesOnly) {
    for (let table of Object.keys(tablesManager)) {
      ...
      logger.info(`Synchronizing ${totalRecordsSource} records in ${table} from DB ${sourceDB} with ${totalRecordsTarget ? `${totalRecordsTarget} on ` : ''}${table} from DB ${targetDB} ${additionalMessage ? additionalMessage : ''}...      `)
      let sourceCursor
      let targetCursor
